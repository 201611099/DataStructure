...
int insertMaxHeap (Heap *heap, element data) { 
// NULL guard
	if (!heap)
	{
...
int	*heapSortAscend(Heap *heap)
{
	int	heapSize = heap->currentElementCount;
	int	*ret;

	ret = (int *)malloc(sizeof(int) * heapSize);
	while (heapSize--)
	{
		ret[heapSize - 1] = heap->pElement[1].key;
		deleteHeap(heap);
	}
	return (ret);
}
